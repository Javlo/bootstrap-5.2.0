...
<c:if test="${!boxBloc}">
<c:if test="${!closeBox}">
<div id="${comp.sectionId}"  class="free-box ${cssClass}">
  <c:if test="${contentContext.previewEditionMode}">
    <div class="card-header border-0"><div id="${previewID}" class="${previewClass}" ${previewData}>[OPEN BOX]</div></div>
  </c:if>
  <c:if test="${not empty titleBox}"><h${contentContext.titleDepth+1} class="box-title">${titleBox}</h${contentContext.titleDepth+1}></c:if>
  <div class="box-free ${comp.style} ${manualCssClass}">
