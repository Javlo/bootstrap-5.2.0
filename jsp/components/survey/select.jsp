...
<c:if test="${not empty question.response}">
	<c:set var="selected" value="${selected}${sep}${question.number}" />
	<c:set var="sep" value="," />
</c:if>
<div class="col-md-6 mb-3">
	<a href="#" class="wizard-list-item card border-left-success h-100 py-2 ${empty question.response?'active':'done'}" onclick="selectClick(this); return false;">
		<div class="card-body">
			<div class="row no-gutters align-items-center">
				<div class="col-sm-auto mr-2 d-flex justify-content-center">
					<i class="undone-item bi bi-toggle-off"></i>
					<i class="done-item bi bi-toggle-on"></i>
				</div>
...
<input id="select-value" type="hidden" name="selected" value="${selected}" />
