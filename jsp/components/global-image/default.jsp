		...
		<c:choose>
			...
			<c:otherwise>
				...
				<figure class="img-${comp.id} ${svg?'svg':'not-svg'} ${not empty param.headerCaption?'balls-zone':''}">
					<c:set var="rel" value="${not empty link?'external':'shadowbox'}" />
					...
					<a <c:if test="${not empty file.imageSize}">data-width="${file.imageSize.width}" data-height="${file.imageSize.height}" data-pswp-width="${file.imageSize.width}" data-pswp-height="${file.imageSize.height}"</c:if> data-caption="${imageAlt}" id="lnk${file.id}" class="${type}" href="${url}" ${titleAttr} ${relAttribute} ${info.openExternalLinkAsPopup && (fn:startsWith(url,'http://') || fn:startsWith(url,'https://') || fn:startsWith(url,'//'))?' target="_blank"':''}>
					<c:if test="${filter != 'raw'}">
							...
							<img id="img-${compid}" data-compid="${compid}" src="${previewURL}" class="img-fluid${info.device.pdf?' return-size':''}" ${imageWidthTag} ${not empty imgCaption?imgCaption:''} alt="${imageAlt}" ${imageStyle} ${imageWidth} ${editPreview?"onload='editPreview.returnSize();'":""} />
						</c:if> <c:if test="${not empty param.headerCaption}">
					...
					<c:if test="${empty param.nolabel}">
						...
						<c:if test="${not empty copyright}">
							<c:set var="copyrightHTML" value='<span class="copyright">${copyright}</span>' />
						</c:if>
						<c:if test="${not empty copyright || not empty label || not empty description}">
							<figcaption>${not empty label?label:description}${copyrightHTML}</figcaption>
						</c:if>
					</c:if>
				</figure>
			</c:otherwise>
